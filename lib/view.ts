...
export function calculateKpis(records: TransactionFact[]): ViewKpis {
  if (!records.length) {
    return { revenue: 0, grossRevenue: 0, profit: 0, margin: 0, discountAmount: 0, discountRate: 0, orders: 0, units: 0, aov: 0, asp: 0, cost: 0 };
  }
  let revenue = 0;
  let grossRevenue = 0;
  let profit = 0;
  ...
  return {
    ...
    discountAmount,
    discountRate: grossRevenue ? discountAmount / grossRevenue : 0,
    orders: orders.size,
    units,
...
export function monthlySeries(records: TransactionFact[]): MonthlyPoint[] {
  ...
  for (const row of records) {
    const bucket = groups.get(row.month) ?? { revenue: 0, profit: 0, gross: 0, discount: 0 };
    bucket.revenue += row.net_revenue;
    bucket.profit += row.gross_profit;
    bucket.gross += row.gross_revenue;
    bucket.discount += row.discount_amount;
    groups.set(row.month, bucket);
  }
  return Array.from(groups.entries())
    ...
    .map(([month, value]) => ({
      ...
      discountRate: value.gross ? value.discount / value.gross : 0,
    }));
}
...
export function productSeries(records: TransactionFact[]): ProductPoint[] {
  ...
  for (const row of records) {
    const bucket = groups.get(row.product) ?? {
      product: row.product,
      revenue: 0,
      profit: 0,
      margin: 0,
  ...
  return Array.from(groups.values())
    .map((row) => ({
      ...
      revenue: row.revenue,
      profit: row.profit,
      margin: row.revenue ? row.profit / row.revenue : 0,
      discountRate: row.gross ? row.discount / row.gross : 0,
      units: row.units,
    }))
...
export function previousPeriodFilters(filters: Filters, minDate: string): Filters | null {
  ...
  const previousStart = new Date(previousEnd.getTime() - durationMs + 86_400_000);
  const min = new Date(`${minDate}T00:00:00Z`);
  if (previousStart < min) return null;
  const iso = (date: Date) => date.toISOString().slice(0, 10);
  return { ...filters, startDate: iso(previousStart), endDate: iso(previousEnd) };
}
